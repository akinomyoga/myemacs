...
(defun mwg-isearch-forward-word/cwb ()
  (let ((p0 (point)))
    (forward-char 1)
    (if (re-search-backward "\\(?:\\(^\\|[^_]\\)\\<[[:alnum:]]\\|\\(^\\|[^_[:alnum:]]\\)_\\)[_[:alnum:]]*?" nil t)
        (if (or (and (match-beginning 1)
                     (/= (match-beginning 1) (match-end 1)))
                (and (match-beginning 2)
                     (/= (match-beginning 2) (match-end 2))))
            (forward-char 1))
      (if (re-search-forward "\\(?:\\<[[:alnum:]]\\|_\\)" nil t)
          (backward-char 1)))))
(defun mwg-isearch-forward-word/cwe ()
  (if (re-search-forward "[_[:alnum:]]*?\\(?:_\\([^_[:alnum:]]\\|$\\)\\|[[:alnum:]]\\>\\([^_]\\|$\\)\\)" nil t)
      (if (or (and (match-beginning 1)
                   (/= (match-beginning 1) (match-end 1)))
              (and (match-beginning 2)
                   (/= (match-beginning 2) (match-end 2))))
